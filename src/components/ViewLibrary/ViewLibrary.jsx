...
export const ViewLibrary = ({ authUser }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [booksResponse, authorsResponse, imagesResponse] =
          await Promise.all([
        ...
        const booksWithAuthors = booksList.map((book) => ({
          ...
          coverUrl:
            book.imageUrl || imagesByTitle[book.title?.toLowerCase()] || null,
        }));
  ...
  const handleDeleteBook = async (id) => {
    if (window.confirm("Are you sure you want to delete this book?")) {
      try {
        ...
        alert(
          "Failed to delete the book: " + (error.message || "Unknown error"),
        );
      }
    }
  };
  ...
  const handleDeleteAuthor = async (id) => {
    if (window.confirm("Are you sure you want to delete this author?")) {
      try {
        ...
        alert(
          "Failed to delete the author: " + (error.message || "Unknown error"),
        );
      }
    }
  };
        ...
        >
          <div className="tabBar">
            ...
            <div className="nav-dropdown">
              <span className="dropdown-trigger">View Library ▼</span>
