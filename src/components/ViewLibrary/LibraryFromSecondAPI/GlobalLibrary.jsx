...
export const GlobalLibrary = ({
  ...
  useEffect(() => {
    const fetchGlobalBooks = async () => {
      try {
        const data = await apiService.get("gutendex/allBooks");
  ...
  const findOrCreateAuthor = async (authorObj) => {
    ...
    const match = normalizedAuthors.find((author) => {
      const label = `${author.firstName} ${author.lastName}`
        .trim()
        .toLowerCase();
    ...
    const birthDate = birthYear ? `${birthYear}` : DEFAULT_BIRTH_DATE;
  ...
  const handleSave = async () => {
    ...
    try {
      const primaryAuthor = selectedBook.authors?.[0] || {
        name: "Unknown Author",
      };
